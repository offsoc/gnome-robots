...
impl GameArea {
    ...
    pub fn set_game(&self, game: Game) {
        ...
        let game_listener_id = game
            .on_game_event
            .connect(glib::clone!(@weak self as this => move |event| this.on_game_event(*event)));
        *this_game = Some(game);
    ...
    fn on_game_event(&self, event: GameEvent) {
        match event {
            ...
            GameEvent::Scored(score) if score > 0 => self.log_score(score),
            GameEvent::Victory => {
    ...
    fn log_score(&self, score: u32) {
        ...
        add_score(&category, score as i64, &window);
    }
